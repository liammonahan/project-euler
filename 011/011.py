...
def get_horizontal(pos):
    ...
    if hits_right_wall(pos):
        return coords
    for x in range(pos, pos + combo_length):
        coords.append(grid[x])
    return coords


def get_right_diagonal(pos):
    coords = []
...
def get_vertical(pos):
    coords = []
    if hits_bottom_wall(pos):
        return coords
    for x in range(4):
...
for x in range(width * height):
    coords.append(get_horizontal(x))
    coords.append(get_right_diagonal(x))
    coords.append(get_left_diagonal(x))
    coords.append(get_vertical(x))
